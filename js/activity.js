...
function createChart(data) {
  ...
  const node = svg.append("g")
    ...
    .append("text")
      ...
      .on("mouseover", overed)
      .on("mouseout", outed)
      // REQUIREMENT 3: Updated Tooltip logic
      .call(text => text.append("title").text(d => `${d.data.name} (${d.parent.data.name})\nIncoming routes: ${d.incoming.length}\nOutgoing routes: ${d.outgoing.length}`));
  ...
  // REQUIREMENT 2: Hover logic
  function overed(_event, d) {
    // Bold the hovered node and connected nodes
    d3.select(this).attr("font-weight", "bold");
    d3.selectAll(d.incoming.map(([source]) => source.text)).attr("font-weight", "bold");
    d3.selectAll(d.outgoing.map(([, target]) => target.text)).attr("font-weight", "bold");

    // Highlight the connected links (paths) and raise them to the front
    d3.selectAll(d.incoming.map(d => d.path))
        .attr("stroke", linkData => airlineColor[linkData[2]] || "#888") // Apply airline color
        .attr("stroke-width", 2)
        .raise();
  ...
  // REQUIREMENT 2: Unhover logic
  function outed(_event, d) {
    // Reset nodes
    d3.select(this).attr("font-weight", null);
    d3.selectAll(d.incoming.map(([source]) => source.text)).attr("font-weight", null);
    d3.selectAll(d.outgoing.map(([, target]) => target.text)).attr("font-weight", null);

    // Reset links by clearing the inline stroke and width
    d3.selectAll(d.incoming.map(d => d.path))
        .attr("stroke", null)
